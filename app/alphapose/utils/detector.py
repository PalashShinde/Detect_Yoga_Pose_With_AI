...
class DetectionLoader():
    def __init__(self, input_source, detector, cfg, opt, mode='image', batchSize=1, queueSize=128):
        ...
        if opt.sp:
            ...
            self.image_queue = mp.Manager().Queue(maxsize=queueSize)
            self.det_queue = mp.Manager().Queue(maxsize=10 * queueSize)
            self.pose_queue = mp.Manager().Queue(maxsize=10 * queueSize)
            ...
            print("INSIDE QEUE",self.pose_queue)
